...
- `src/otari_agent_gates/otari-plugin.toml`: the manifest Otari reads before importing anything.
- `src/otari_agent_gates/__init__.py`: `register(ctx)`, the only thing Otari calls.
- `models.py`: gate and policy schemas, the `plugins.agent-gates` config model, and the two tables on the plugin's own `Base`.
- `service.py`, `routes.py`, `cli.py`, `transcript.py`, `headless.py`: evaluation, the API, `otari policy`, transcript extraction, the `claude` CLI wrapper.
- `migrations/`: the plugin's Alembic chain, stamped in `alembic_version_agent_gates`. Never edit a shipped revision; add a new one.
- `hooks/`: the two Claude Code hook scripts. Standard library only, and thin: they shell out to `otari policy ...`.
...
- Route paths are relative to the mount Otari supplies (`/api/v1/plugins/agent-gates`); the routers own only `/policy-checks`.
